...
/**
 * Verifica que la suma de hectáreas de las parcelas de una finca no supere
 * el área de la finca. excludeParcelaId permite excluir una parcela durante
 * un update (para que no se sume a sí misma).
 */
async function validateAreaParcelasFinca(
  fincaId: number,
  hectareasNuevas: number,
  excludeParcelaId?: number,
): Promise<void> {
  const fincaRes = await pool.query(
    `SELECT nombre, area::float AS area FROM fincas WHERE id = $1`,
    [fincaId],
  );
  if (fincaRes.rows.length === 0) {
    throw new Error("La finca indicada no existe");
  }
  const finca = fincaRes.rows[0] as { nombre: string; area: number };

  const sumQuery = excludeParcelaId
    ? `SELECT COALESCE(SUM(hectareas)::float, 0) AS suma FROM parcelas WHERE finca_id = $1 AND id <> $2`
    : `SELECT COALESCE(SUM(hectareas)::float, 0) AS suma FROM parcelas WHERE finca_id = $1`;
  const params = excludeParcelaId ? [fincaId, excludeParcelaId] : [fincaId];
  const sumRes = await pool.query(sumQuery, params);
  const ocupado = Number(sumRes.rows[0].suma);
  const total = Number(finca.area);
  const disponible = total - ocupado;

  if (hectareasNuevas > disponible + 0.0001) {
    throw new Error(
      `La parcela no cabe en la finca "${finca.nombre}". ` +
        `Área total: ${total.toFixed(2)} ha, ocupada por otras parcelas: ${ocupado.toFixed(2)} ha, ` +
        `disponible: ${disponible.toFixed(2)} ha. Estás intentando registrar ${hectareasNuevas.toFixed(2)} ha.`,
    );
  }
}
...
export class FarmService {
  ...
  async updateFinca(
    ...
    // Si se cambia el área, validar que las parcelas existentes sigan cabiendo.
    if (data.area !== undefined) {
      ...
      const ocupado = Number(sumRes.rows[0].suma);
      if (data.area < ocupado - 0.0001) {
        throw new Error(
          `No puedes reducir el área a ${data.area.toFixed(2)} ha. ` +
            `Las parcelas existentes ya ocupan ${ocupado.toFixed(2)} ha. ` +
            `Para reducir el área de la finca, primero elimina o redimensiona las parcelas.`
        );
      }
    }
  ...
  async createParcela(propietarioId: number, data: {
    ...
    // Validar que la nueva parcela quepa en el área de la finca
    await validateAreaParcelasFinca(data.fincaId, data.hectareas);
  ...
  async updateParcela(parcelaId: number, propietarioId: number, data: Partial<{
    ...
    // Si se están cambiando hectáreas, validar que la suma con las demás parcelas
    // no exceda el área de la finca, y que los cultivos existentes sigan cabiendo.
    if (data.hectareas !== undefined) {
      ...
      if (fincaRes.rows.length === 0) return null;
      await validateAreaParcelasFinca(fincaRes.rows[0].finca_id, data.hectareas, parcelaId);

      // Verificar que los cultivos existentes (en m²) sigan cabiendo en la nueva área.
      // 1 ha = 10 000 m².
      const sumCultivosRes = await pool.query(
        `SELECT COALESCE(SUM(area_m2)::float, 0) AS suma FROM cultivos WHERE parcela_id = $1`,
        [parcelaId]
      );
      const ocupadoM2 = Number(sumCultivosRes.rows[0].suma);
      const nuevoTotalM2 = Number(data.hectareas) * 10000;
      if (nuevoTotalM2 < ocupadoM2 - 0.01) {
        throw new Error(
          `No puedes reducir el área a ${data.hectareas.toFixed(2)} ha (${nuevoTotalM2.toFixed(0)} m²). ` +
            `Los cultivos existentes ocupan ${ocupadoM2.toFixed(0)} m². ` +
            `Elimina o redimensiona los cultivos antes de reducir la parcela.`
        );
      }
    }
