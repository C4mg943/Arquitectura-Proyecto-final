  ...
  app.get("/api/notifications", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const result = await pool.query(
        "SELECT id, user_id, tipo, titulo, mensaje, leida, created_at FROM notificaciones WHERE user_id = $1 ORDER BY created_at DESC LIMIT 100",
        [userId]
      );
      // Mapeamos snake_case -> camelCase para que el frontend lo consuma directo
