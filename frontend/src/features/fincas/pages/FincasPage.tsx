...
export default function FincasPage({ adminMode = false }: { adminMode?: boolean } = {}) {
  ...
  // Al abrir edición, cargar el uso de área de la finca para limitar el mínimo permitido
  useEffect(() => {
    if (!editing) {
      setAreaUsage(null)
      return
      ...
      {isFormOpen ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/45 p-4">
          <div className="w-full max-w-3xl rounded-3xl bg-surface p-6 shadow-[0_20px_60px_rgb(0_0_0_/_30%)]">
            ...
            {error ? (
              <div className="mt-4 rounded-xl bg-error-container px-3 py-2.5 text-sm font-semibold text-on-error-container">
                {error}
              </div>
            ) : null}

            {/* Indicador de uso de área (solo en edición) */}
            {editing && areaUsage ? (() => {
              const nuevaArea = Number(form.area)
              const ocupado = areaUsage.ocupadoHa
              const excede = !Number.isNaN(nuevaArea) && nuevaArea < ocupado - 0.0001
              return (
                <div className={`mt-4 rounded-xl px-3 py-2.5 text-sm ${excede ? 'bg-error-container text-on-error-container' : 'bg-primary-container text-on-primary-container'}`}>
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <span className="font-semibold">
                      {excede ? '⚠ El área no puede ser menor a lo ocupado por las parcelas' : '📐 Espacio ocupado por las parcelas'}
                    </span>
                    <span className="text-xs">
                      Ocupado: <strong>{ocupado.toFixed(2)} ha</strong> · Mínimo permitido: <strong>{ocupado.toFixed(2)} ha</strong>
                    </span>
                  </div>
                  {excede ? (
                    <p className="mt-1 text-xs">
                      Para reducir el área a {nuevaArea.toFixed(2)} ha, primero elimina o redimensiona las parcelas dentro de la finca.
                    </p>
                  ) : null}
                </div>
              )
